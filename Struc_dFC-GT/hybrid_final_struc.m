...
if length(varargin)<6   
    ...
    while max(abs(error))> .99% loop to solve for TIT by varying the recovery percentage of o2
        ...
        %% Initialization for Fuel Cell
        T6 = zeros(length(T1),1)+1023;
        ASR = zeros(length(T1),1)+2;        %Design Area Specific Resistance
        eff_2 = zeros(length(T1),1)+.6;      %Effectiveness of WGS
        Oxidant.O2 = N3;
        L = zeros(length(T1),1)+10;     %Length of cells
        W = zeros(length(T1),1)+10;     %Width of cells
        n = zeros(length(T1),1)+10;     %Nodes to analyze
        Cells = zeros(length(T1),1)+1e5;
        %% Fuel Cell Model
        [i,recirc,FC_Fuel,FlowOut,V,W_fc] = FuelCell_struc(T6,ASR,eff_2,S2C,Oxidant,L,W,n,Cells,Pr);         
        %% Initialize Combustor
        LHVanode = zeros(length(T1),1)+LHVfuel.*X6.CH4+LHVCO.*X6.CO+LHVH2.*X6.H2;%LHV of anode off products
        %% Combustor
        [ReactMix,Qextra] = combust_struc(NonPerm,FC_Fuel,Q_preheat,TIT);
%         [T8,X8,N8,~] = combust(T7,X7,N7, Tfuel,X6,N6,Q_preheat,TIT); %No fuel combustor
 ...
 %% Initialization to Fuel Cell
     ...
     for k = 1:1:length(Oxidant.O2)
        ...
        W_vec(k,:) = W_fc;
        TFlowOut(:,k) = FlowOut;
        TotalFlow(k) = NetFlow(FlowOut).*Cells;
     end
     %% Initialize Combustor
     for j = 1:1:length(T1)
         TotalFlow_H2 = TFlowOut.H2(j)*Cells;
         TotalFlow_H2O = TFlowOut.H2O(j)*Cells;
         TotalFlow_CO = TFlowOut.CO(j)*Cells;
         TotalFlow_CO2 = TFlowOut.CO2(j)*Cells;
         TotalFlow_CH4 = TFlowOut.CH4(j)*Cells;
     end
     LHVanode = zeros(length(T1),1)+(TotalFlow_H2./TotalFlow).*LHVH2+...
         (TotalFlow_CO./TotalFlow).*LHVCO+...
         (TotalFlow_CH4./TotalFlow)*LHVfuel;%LHV of anode off products
     %% Combustor Model
     for q = 1:1:length(Q_preheat)
        Air.T = NonPerm.T(k);
        Air.O2 = NonPerm.O2(k);
        Air.N2 = NonPerm.N2(k);
        Fuel.T = TotalFlow.T(k);
        Fuel.CH4 = TotalFlow.CH4(k);
        Fuel.H2 = TotalFlow.H2(k);
        Fuel.CO = TotalFlow.CO(k);
        [ReactMix, Qextra] = combust_struc(Air,Fuel,Q_preheat,TIT);
     end
end
%% Turbine Model
[Wt,T_out] = turbine_struc(ReactMix,TurbEff, Pr);
%% Results
W_gt = Wt-Wc1;  %Gas turbine power minus parasitic compression%
Eff_FC = W_fc./(FC_Fuel*LHVfuel);
Eff_GT = (Wt-Wc1)./((FlowOut.*LHVanode)); %Efficiency of Gas Turbine
W_net = W_fc + W_gt - Wc2;  %Net power output of hybrid
%% Decide whether cogeneration of h2 is wanted
Cogen = 0; 
if Cogen == 0
    Efficiency = (W_net)./((FC_fuel)).*LHVfuel-Qextra; %Hybrid Efficiency without cogeneration
else
    Efficiency = (W_net+(FlowOut.H2.*LHVH2))./((FC_fuel).*LHVfuel-Qextra);%hybrid efficiency with cogeneration
end
