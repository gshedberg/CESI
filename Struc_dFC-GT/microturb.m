function [Efficiency,Eff_FC,Eff_GT,W_net,Wfc_vec,W_gt,Wc2,T_out,FCFlowOut,ReactMix,V_vec,Utilization,R_actual,Rt,recovery,Qextra,i_array,recirc_vec] = microturb(varargin)
...
while max(abs(error))> 1e-2% loop to solve for TIT by varying the recovery percentage of o2
    ...
    if ~exist('T_out','var')
           TurbFlow = CompFlow;
           TurbFlow.T = ones(length(Tin),1).*900;
    end
        [TurbFlow2,CompFlow2] =HX_struct(CompFlow,TurbFlow,.85);
    %% OTM for Oxygen Separation
    [NonPerm,O2Flow,Q_preheat,R_actual,Rt] = ITM_struc(CompFlow2,P2,P_ITMperm,recovery); %ITM Model
