function [Wc,T_out,Xout,Nout, P2,Q_transfer] = compress_heatex(TXNin, nc, Pr, Pin)
Ru = 8.314;
...
% if ~isempty(A)
    ...
    T_out = T_hotin - dT;
    [~,H_OTMin] = enthalpy(T_out,Xout,Nout);
    Q_transfer = Nout.*(H_OTMin-H2);
...
%         T_out(B)  = T_out(B) + T_error(B); %Reiteration to calculate temperature out
%     end
%     
% end
