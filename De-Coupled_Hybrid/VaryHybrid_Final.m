...
if m ==1  %decision on whether to run constant vs varying recovery
    ...
    for j= 1:n
        ...
        if Fuel==1
            [Efficiency(:,j),Eff_FC(:,j),Eff_GT(:,j),W_net(:,j),W_fc(:,j),W_gt(:,j),T_out(:,j),X8,N_out(:,j),V_fc,R_actual(:,j),recovery(:,j),FC_util(:,j),Qimbalance(:,j),E0(:,j),n_h2(:,j),Tstoich(:,j)] = hybrid_final(TXNin,Pr, P_ITMperm, V_loss,TIT,recovery);
        else
            [Efficiency(:,j),Eff_FC(:,j),Eff_GT(:,j),W_net(:,j),W_fc(:,j),W_gt(:,j),T_out(:,j),X8,N_out(:,j),V_fc(:,j),R_actual(:,j),recovery(:,j),FC_util(:,j),Qimbalance(:,j),E0(:,j),n_h2(:,j),Tstoich(:,j)] = hybrid_final(TXNin, Pr, P_ITMperm, V_loss,TIT);
        end
    end
    x = V_fc;
    y = linspace(.0099,.99);
    ...
    if Fuel==1
        [Efficiency,Eff_FC,Eff_GT,W_net,W_fc,W_gt,T_out,X8,N_out,V_fc,R_actual,recovery,FC_util,Qimbalance,E0,n_h2,Tstoich] = hybrid_final(TXNin,Pr, P_ITMperm, V_loss,TIT,recovery);
    else
        [Efficiency,Eff_FC,Eff_GT,W_net,W_fc,W_gt,T_out,X8,N_out,V_fc,R_actual,recovery,FC_util,Qimbalance,E0,n_h2,Tstoich] = hybrid_final(TXNin, Pr, P_ITMperm, V_loss,TIT);
    end
    figure(1)
    A = find(recovery>.99);
    if ~isempty(A)
...
%     xlabel('Voltage of FC')
%     ylabel(hAx(1),'Efficiency of Hybrid') % left y-axis
%     ylabel(hAx(2),'Efficiency of Fuel Cell') % left y-axis
%     hLine1.LineStyle = '--';
%     hLine2.LineStyle = ':';
