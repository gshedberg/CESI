...
if m ==1  %decision on whether to run constant vs varying recovery
    ...
    if Fuel==1
        [Efficiency,Eff_FC,Eff_GT,W_net,W_fc,W_gt,T_out,X8,N_out,V_fc,R_actual,recovery,FC_util,Qimbalance,E0,n_h2] = hybrid_final(TXNin,Pr, P_ITMperm, V_loss,TIT,recovery);
    else
        [Efficiency,Eff_FC,Eff_GT,W_net,W_fc,W_gt,T_out,X8,N_out,V_fc,R_actual,recovery,FC_util,Qimbalance,E0,n_h2] = hybrid_final(TXNin, Pr, P_ITMperm, V_loss,TIT);
    end
    figure(1)
    A = find(recovery>.99);
    if ~isempty(A)
