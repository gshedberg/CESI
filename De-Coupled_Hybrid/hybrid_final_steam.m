function [Efficiency,Eff_FC,Eff_GT,W_net,W_fc,W_gt,T_out,X8,N9,V_fc,R_actual,recovery,FC_util,Qimbalance,E0,n_h2] = hybrid_final_steam(varargin)
...
[Wt,T9, N9] = turbine([T8,X8,N8], TurbEff, 1./Pr);
Twater = ones(length(T9),1).*295;
Xwater = zeros(length(X7),7);
Xwater(:,5) = 1;
Nwater = ones(length(N6),1).*.5;
Eff_pump = .90;
[T10,X10,N10,W_pump] = steam([T9,X8,N9],[Twater,Xwater,Nwater],Eff_pump,Pr);
[Wt_steam,T_out, N_out] = turbine([T10,X10,N10], TurbEff, 1./Pr);

W_gt = Wt-Wc1;  %Gas turbine power minus parasitic compression%
Eff_GT = (Wt-Wc1)./((N6.*LHVanode)+Q_preheat); %Efficiency of Gas Turbine
W_net = W_fc + W_gt +Wt_steam - Wc2 - W_pump;  %Net power output of hybrid
